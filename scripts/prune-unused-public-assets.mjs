...
const textExtensions = new Set([
  '.css', '.html', '.htm', '.js', '.jsx', '.mjs', '.cjs', '.ts', '.tsx',
  '.json', '.gltf', '.mtl', '.obj', '.svg', '.xml', '.txt', '.md', '.webmanifest',
]);
const ignoredDirectories = new Set(['.git', 'dist', 'node_modules', 'coverage']);
const ignoredFiles = new Set([
  'package-lock.json', 'pnpm-lock.yaml', 'yarn.lock', 'asset-prune-report.json',
]);
...
const projectFiles = await walkFiles(projectRoot, (name) => !ignoredDirectories.has(name));
...
for (const match of corpus.matchAll(/["'`](\/?(?:[\w.@%+(), -]+\/)+)[^"'`\n]*?\$\{/g)) {
...
for (const publicFile of publicFiles) {
  ...
  const isReferenced = corpus.includes(relative)
...
console.log(`[asset-prune] ${path.basename(projectRoot)}: removed ${removedFiles} unused files (${formatMb(removedBytes)}).`);
console.log(`[asset-prune] Public payload: ${formatMb(publicBytes)} -> ${formatMb(keptBytes)}.`);
